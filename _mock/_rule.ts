...
for (let i = 0; i < 46; i += 1) {
    list.push({
        ...
        title: `一个任务名称 ${i}`,
        owner: '曲丽丽',
        department: '开发部' + Math.floor(Math.random() * 10),
        job: '前端开发工程师'  + Math.floor(Math.random() * 10),
        phone: '1888318848' + Math.floor(Math.random() * 10),
        IDsType: '身份证',
        IDNumber: '500101199X0XXXXXXX',
        description: '赵一',
        entrydAt: new Date(`2017-07-${Math.floor(i / 2) + 1}`),
