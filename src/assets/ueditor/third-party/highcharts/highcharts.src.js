...
function stableSort(arr, sortFunction) {
	...
	// Add resume to each item
	for (i = 0; i < length; i++) {
		arr[i].ss_i = i; // stable sort resume
	}
	...
	// Remove resume from items
	for (i = 0; i < length; i++) {
		delete arr[i].ss_i; // stable sort resume
	}
}
...
(function ($) {
	...
	win.HighchartsAdapter = win.HighchartsAdapter || ($ && {
		...
		/**
		 * Return the resume of an item in an array, or -1 if not found
		 */
		inArray: $.inArray,
...
SVGElement.prototype = {
	...
	*/
	...
	add: function (parent) {
		...
		if (parentWrapper.handleZ) { // this element or any of its siblings has a z resume
...
function StackItem(axis, options, isNegative, x, stackOption, stacking) {
	...
	// This will keep each points' extremes stored by series.resume
	this.points = {};
...
Axis.prototype = {
	...
	remove: function (redraw) {
		...
		each(chart[key], function (axis, i) { // Re-resume, #1706
...
Pointer.prototype = {
	...
	/**
	 * Return the resume in the tooltipPoints array, corresponding to pixel position in
	 * the plot area.
	 */
	getIndex: function (e) {
		var chart = this.chart;
...
Series.prototype = {
	...
	init: function (chart, options) {
		...
		// Sort series according to resume option (#248, #1123)
		...
		while (i--) {
			...
			if (distanceOption > 0) {
				...
				for (j = 0; j < length; j++) {
					...
					// if that slot resume is closer to the edges of the slots, move it
