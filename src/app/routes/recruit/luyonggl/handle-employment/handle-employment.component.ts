...
@Component({
  selector: 'handle-employment',
  templateUrl: './handle-employment.component.html',
  styleUrls: ['./handle-employment.component.css']
})
export class HandleEmploymentComponent implements OnInit {
  ...
  private resumeUrl = environment.SERVER_URL + '/recruit';
  public optionList = [];
  // 证件类型
  public selectData0 = [];
  // 试用期限
  ...
  _submitForm() {
    ...
    obj['employApprovalId'] = this.ymguid;
    if (this.typeNumber === '1') {
      this.http.post(this.url + 'service/waitingEntry/2', obj, { headers: headers }).subscribe((res: any) => {
        if (res.code === 0) {
        ...
        if (res.code === 0) {
          res.data.forEach((value: any, key: any) => {
            ...
            if (value.dictCode === 'DIC_PROBATION_PERIOD') {
              ...
              // console.log(this.probationTime);
            }
...
*/
  ...
  edidGetData(guid) {
    this.http.get(this.resumeUrl + '/service/employApproval/' + guid).subscribe((res: any) => {
      if (res.code === 0) {
        ...
        this.form.setValue({
          ...
          departmentId: this.formData.entryDepartmentId,
          position: this.formData.entryPostion,
          // isManager: ['0'],
          // jobNumber: 'this.formData.jobNumber',
          jobNumber: '',
          entryTime: this.strTransFormDate(this.formData.entryDateTime),
          workType: '0' + this.formData.workNature,
          fullmemMoney: this.formData.payRegularWorker,
          trialMoney: this.formData.payOnTrial,
          probationTime: this.formData.probationStage,
  ...
  ngOnInit(): void {
    ...
    if (this.ymguid !== undefined || this.ymguid !== '') {
