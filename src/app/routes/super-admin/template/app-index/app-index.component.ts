...
* */
    ...
    * */
    public getListDatas() {
        this.http.get(this.url + '/app-template-menu/' + this.guid)
            .subscribe((res: any) => {
                if (res.code === 1) {
                    this.dataList = [];
...
* */
    submit() {
        const parentMenuIds = [];
        const subMenuIds = [];
        this.dataLists.forEach((value: any) => {
            if (value.isDefault) {
        ...
        this.http.put(this.url + '/app-template-menu/' + this.guid, {
