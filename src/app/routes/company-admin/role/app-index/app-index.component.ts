...
export class AppIndexComponent implements OnInit {
   ...
   * */
    public getData() {
        ...
        this.http.get(this.url + '/app-role/page-list', {params: this.q})
            .subscribe((res: any) => {
                if (res.code === 1) {
    ...
    * */
    ...
    handleOkMiddle(): void {
        this.isVisibleMiddle = true;
        this.http.delete(this.url + '/app-role/' + this.guid)
            .subscribe((res: any) => {
                if (res.code === 1) {
    ...
    changeRoleState(item) {
        this.modal.confirm({
            nzTitle: '变更状态',
            nzContent: `是否${ item.roleState === 1 ? '关闭' : '开启'}该权限`,
            nzOkText: '确定',
            nzCancelText: '取消',
            nzOnOk: () => {
