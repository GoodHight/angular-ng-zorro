...
export class IndexComponent implements OnInit {
  ...
  public key = '';
  public starttime = new Date(); // 开始日期
  public endtime = new Date(); // 结束日期
  public starttimeCorrect = new Date(); // 开始日期
  public endtimeCorrect = new Date(); // 结束日期
  ...
  * */
  ...
  getData(): void {
    ...
    this.statisticsService.getData(this.queryParams)
  ...
  export(): void {
    this.statisticsService.export(this.queryParams);
  }
  ...
  check31Days(): boolean {
    ...
    if (this.endtime.valueOf() - this.starttime.valueOf() < 3600 * 24 * 180 * 1000) {
      return false;
    }
    return true;
  }
  ...
  onChange(result: Date, type: string) {
    ...
    if (!this.check31Days()) {
      ...
      this.msg.error('开始时间与结束时间间隔不能超过180天.');
      if (type === 'start') {
        this.starttime = this.starttimeCorrect;
      } else {
        this.endtime = this.endtimeCorrect;
