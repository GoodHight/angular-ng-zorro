...
export class NoticeEditComponent implements OnInit {
    ...
    public selectMessageType = '01'; // 当前选中select
    public title = ''; // 标题
    messageIsTop = false;
    ...
    public submit(type: any) {
        ...
        let propsUrl = this.http.post(environment.NOTICE_URL + 'service/notice/1', body);
        if (this.uuid && this.uuid.length > 0) {
            propsUrl = this.http.patch(environment.NOTICE_URL + 'service/notice/1/' + this.uuid, body);
        }
    ...
    onClose(index): void {
        this.selectPeopleData.splice(index, 1);
